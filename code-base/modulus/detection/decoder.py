import tensorflow as tf
from modulus.detection.utilities import *
from modulus.detection.anchor_boxes_generator import *


class Decoder:

    def __init__(self, num_classes=80,
                 confidence_threshold=0.005, nms_iou_threshold=0.5,
                 max_detections_per_class=100, max_detections=100,
                 box_variance=[0.1, 0.1, 0.2, 0.2], **kwargs):
        self.num_classes = num_classes
        self.confidence_threshold = confidence_threshold
        self.nms_iou_threshold = nms_iou_threshold
        self.max_detections_per_class = max_detections_per_class
        self.max_detections = max_detections
        self.box_variance = tf.convert_to_tensor(box_variance, dtype=tf.float32)
        self.anchor_boxes_generator = AnchorBoxesGenerator()

    def decode_box_predictions(self, anchor_boxes, box_predictions):
        boxes = box_predictions * self.box_variance
        boxes = tf.concat([
            boxes[:, :, :2] * anchor_boxes[:, :, 2:] + anchor_boxes[:, :, :2],
            tf.math.exp(boxes[:, :, 2:]) * anchor_boxes[:, :, 2:],
        ], axis=-1)
        boxes = center_to_corner(boxes)
        return boxes

    def decode(self, images, predictions):
        image_shape = tf.cast(tf.shape(images), dtype=tf.float32)
        anchor_boxes = self.anchor_boxes_generator.get_anchor_boxes(image_shape[1], image_shape[2])
        box_predictions = predictions[:, :, :4]
        boxes = self.decode_box_predictions(anchor_boxes[None, :, :], box_predictions)
        boxes = tf.expand_dims(boxes, axis=2)
        cls_predictions = tf.nn.sigmoid(predictions[:, :, 4:])
        return tf.image.combined_non_max_suppression(
            boxes,
            cls_predictions,
            self.max_detections_per_class,
            self.max_detections,
            self.nms_iou_threshold,
            self.confidence_threshold,
            clip_boxes=False,
        )
